...
import math
import subprocess
from statistics import fmean as mean, harmonic_mean
...
def aggregate_evaluation(outputs: List[Dict[str, float]]) -> Dict[str, float]:
    outputs = {key: mean(out[key] for out in outputs) for key in outputs[0]}        
    outputs['harmonic-mean'] = harmonic_mean(
        outputs[key] 
        for key in {'ref-BLEU', 'self-BLEU', 'accuracy'}
        if key in outputs
    )
    return outputs
