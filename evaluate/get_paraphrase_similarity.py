...
from .similarity.test_sim import find_similarity
...
for rp, rs in zip(args.reference_paths.split(","), args.reference_strs.split(",")):
    ...
    print("Avg similarity score vs {} = {:4f}".format(rs, np.mean(sim_scores)), file=sys.stderr)
...
print("Avg max score = {:4f}".format(np.mean(max_scores)), file=sys.stderr)
print(np.mean(max_scores) * 100)
